package org.zeroj.examples.dsl.common;
...
/**
 * Exports witness values to the standard circom/snarkjs `.wtns` format.
 *
 * <p>The .wtns format is the standard circom witness file format used by snarkjs-compatible tooling.</p>
 *
 * <p>ADR-0044: this class was preserved from the removed {@code zeroj-prover-wasm} incubator module.
 * It carries no WASM dependency and exists here only so the integration tests can keep feeding
 * witnesses to the independent snarkjs prover, which is one of ZeroJ's differential oracles. It is
 * deliberately <b>not</b> part of any published module: ADR-0044 removes {@code zeroj-prover-wasm}
 * with no replacement and does not authorize adding this API to a product artifact.</p>
 */
