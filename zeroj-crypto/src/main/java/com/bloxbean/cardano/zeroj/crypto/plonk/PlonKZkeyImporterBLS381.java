...
public final class PlonKZkeyImporterBLS381 {
    ...
    private static final BigInteger FP_R = BigInteger.ONE.shiftLeft(384).mod(MontFp381.modulus());
    private static final BigInteger FP_R_INV = FP_R.modInverse(MontFp381.modulus());

    // Montgomery R for Fr (scalar field) — for converting polynomial evaluations
    private static final BigInteger FR_R = BigInteger.ONE.shiftLeft(256).mod(MontFr381.modulus());
    private static final BigInteger FR_R_INV = FR_R.modInverse(MontFr381.modulus());
    ...
    private static MontFr381 readFrMont(ByteBuffer buf, int n8r, BigInteger rField) {
