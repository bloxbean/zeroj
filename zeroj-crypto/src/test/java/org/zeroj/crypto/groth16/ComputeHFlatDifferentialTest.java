...
class ComputeHFlatDifferentialTest {
    ...
    void flatIO_roundTrip(@org.junit.jupiter.api.io.TempDir java.nio.file.Path dir) throws Exception {
        ...
        org.zeroj.api.R1CSFlatIO.write(flat, "c300-w302-p1", file);
        var back = org.zeroj.api.R1CSFlatIO.readIfMatches(file, "c300-w302-p1");
        assertNotNull(back, "matching fingerprint must load");
        assertNull(org.zeroj.api.R1CSFlatIO.readIfMatches(file, "c1-w2-p3"),
                "fingerprint mismatch must be a cache miss");
        assertNull(org.zeroj.api.R1CSFlatIO.readIfMatches(dir.resolve("absent.bin"), "x"),
        ...
        try (var arena = java.lang.foreign.Arena.ofShared()) {
            var mapped = org.zeroj.api.R1CSFlatIO.readMapped(file, "c300-w302-p1", arena);
            assertNotNull(mapped, "matching fingerprint must map");
            assertNull(org.zeroj.api.R1CSFlatIO.readMapped(file, "c1-w2-p3", arena),
