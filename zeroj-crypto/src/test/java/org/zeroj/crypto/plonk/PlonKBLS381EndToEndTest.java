...
import org.zeroj.crypto.transcript.FiatShamirTranscript;
...
/**
 * End-to-end PlonK test for BLS12-381: CircuitBuilder → compile → setup → prove → verify.
 */
class PlonKBLS381EndToEndTest {

    private static final BigInteger FR = MontFr381.modulus();

    @Test
    void fullPipeline_multiplier_proveAndVerify() {
        // === Step 1: Define circuit via DSL ===
        ...
        assertNotNull(proof);
        assertTrue(proof.commitA().isOnCurve(), "commitA on curve");
        assertTrue(proof.commitZ().isOnCurve(), "commitZ on curve");
        assertFalse(proof.commitA().isInfinity(), "commitA not infinity");

        System.out.println("PlonK BLS12-381 proof generated (pure Java):");
        ...
        // PI
        BigInteger piVal = BigInteger.ZERO;
        ...
        for (int i = 0; i < pubInputs.length; i++) {
            ...
            piVal = piVal.subtract(pubInputs[i].multiply(Li).mod(FR)).mod(FR);
        ...
        BigInteger r0 = piVal.subtract(e2).mod(FR).subtract(e3).mod(FR);
        ...
        G1Point G1gen = toG1(JacobianG1BLS381.GENERATOR.toAffine());
        G1Point E_point = G1gen.scalarMul(eScalar);

        G1Point lhs = F.add(E_point.negate())
                .add(Wxi.scalarMul(xi))
                .add(Wxiw.scalarMul(u.multiply(xi).mod(FR).multiply(omega).mod(FR)));
        G1Point rhs_g1 = Wxi.add(Wxiw.scalarMul(u));

        G2Point G2gen = toG2(JacobianG2BLS381.GENERATOR.toAffine());

        boolean verified = BLS12381Pairing.pairingCheck(
                new G1Point[]{lhs, rhs_g1.negate()},
                new G2Point[]{G2gen, X_2});

        assertTrue(verified, "PlonK BLS12-381 pairing verification MUST pass");
        System.out.println("PlonK BLS12-381 end-to-end PROVED + VERIFIED (pure Java pairing check)!");
    }
