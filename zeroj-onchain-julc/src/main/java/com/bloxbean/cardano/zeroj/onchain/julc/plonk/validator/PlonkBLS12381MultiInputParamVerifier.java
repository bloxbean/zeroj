...
/**
 * Cardano-profile BLS12-381 PlonK verifier with script-parameter public inputs.
 *
 * <p>This verifier uses the {@code zeroj-plonk-bls12381-cardano-mpi-v1-json}
 * transcript profile. It is equivalent to {@link PlonkBLS12381MultiInputVerifier}
 * except the ordered public input scalars are pinned as a script parameter
 * instead of supplied by datum.</p>
 */
@SpendingValidator
public class PlonkBLS12381MultiInputParamVerifier {
    ...
    @Param static PlutusData publicInputs;
    ...
    static boolean validate(PlutusData datum, PlonkProof p, PlutusData ctx) {
        ...
        if (!validParams(publicInputCount) || !validProofShapeAndScalars(p)) {
            return false;
        }
    ...
    private static boolean validParams(BigInteger publicInputCount) {
        return fr.signum() > 0
