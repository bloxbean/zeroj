 ...
 *
 * <p>This verifier uses compressed G1 bytes in the Fiat-Shamir transcript, then
 * uncompresses the same canonical bytes for Plutus BLS builtins. It performs
 * the full batched KZG opening pairing check for the ZeroJ BLS12-381 PlonK
 * prover profile.</p>
 */
@SpendingValidator
public class PlonkBLS12381Verifier {
    ...
    static boolean validate(PlutusData datum, PlonkProof p, PlutusData ctx) {
        ...
        BigInteger beta = challenge(cat(
                ...
                cat(cat(vkS3, i2bs(pub0)), cat(p.cmA(), cat(p.cmB(), p.cmC())))));
        ...
        if (!xiMinusOne.multiply(p.xiMinusOneInv()).mod(fr).equals(BigInteger.ONE)
                || !xiMinusOmega.multiply(p.xiMinusOmegaInv()).mod(fr).equals(BigInteger.ONE)) {
            return false;
        }
