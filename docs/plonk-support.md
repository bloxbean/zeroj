...
ZeroJ supports PlonK proof generation via the pure Java prover, plus **pure Java
verification** for structured snarkjs/ZeroJ PlonK proof JSON on BLS12-381.
BN254 PlonK remains a legacy/off-chain compatibility path and is disabled by
default behind explicit opt-in. The Julc on-chain PlonK path is
...
- **Prove**: Generate PlonK proofs with the pure Java prover
- **Verify**: **Pure Java verification** for structured snarkjs/ZeroJ proof JSON
...
2. **gnark binary artifacts**: ZeroJ never shipped a decoder for gnark's opaque binary PlonK proof format, and [ADR-0044](adr/0044-focused-module-surface-and-optional-provider-isolation.md) removed the gnark runtime that could verify it natively. Use the structured snarkjs/ZeroJ proof JSON path.
...
The BLS12-381 PlonK vectors under
`zeroj-test-vectors/src/main/resources/test-vectors/plonk-bls12381/` are produced
by an **independent implementation** — gnark v0.14.0 — not by ZeroJ. They are
what `GnarkTranscriptCompatTest` checks ZeroJ's Fiat-Shamir transcript against.

[ADR-0044](adr/0044-focused-module-surface-and-optional-provider-isolation.md)
removed the gnark runtime provider but preserved the pinned generator so those
vectors stay reproducible:

```bash
cd assurance/gnark-fixtures
make gen        # regenerate in place; then re-run :zeroj-verifier-plonk:test
```

The generator is not byte-reproducible — its SRS is randomized `unsafekzg` — so
the property being checked is that ZeroJ still accepts and agrees with a *fresh*
independent artifact, not that the bytes match. See
`assurance/gnark-fixtures/README.md`.
