...
The pure Java prover does NOT perform trusted setup. It imports proving keys generated by established tools:

| Source | Format | Import method |
|--------|--------|---------------|
| snarkjs | `.zkey` file (iden3 binary) | `ProvingKeyImporter.fromSnarkjs(path)` |
| gnark | gnark binary format | `ProvingKeyImporter.fromGnark(path)` |
| Powers of Tau | `.ptau` file | `SRS.fromPowersOfTau(path)` |

This is a critical safety decision: trusted setup is security-critical and should be done by established, audited tools. The pure Java prover only consumes the output.
