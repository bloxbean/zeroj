...
| `InCircuitPedersen` | Low-level Jubjub Pedersen commitment (in-circuit) | Requires BLS12-381 scalar field | Direct | Yes, pending external review | Commits the residue represented by the supplied scalar bits; it does not assert integer `< l` canonicality. Callers that need canonical integer binding must establish it separately. 3,020 constraints at two 252-bit scalars. Unaffected by the off-circuit timing finding below. |
| `ZkPedersen` | Symbolic Jubjub Pedersen commitment adapter | Requires BLS12-381 scalar field | Direct | Yes, pending external review | Asserts both scalars `< l` through the sound range-constrained comparator, and reuses owned decompositions at each scalar's declared width. |
| `PedersenCommitment.commit` (off-circuit) | Host-side commitment generation | Jubjub over BLS12-381 scalar field | Not a circuit gadget | **Offline/isolated generation only (ADR-0038)** | Both secret scalars use a fresh multiple-of-`l` blinding and fixed 316-add/316-double schedule. The former loop-bound/operation-count channel is removed and the raw trailing-zero signal is blinded, but Java branches and `BigInteger` remain variable-time. The commitment's hiding and binding are unaffected; the restriction is on secret-bearing execution. |
