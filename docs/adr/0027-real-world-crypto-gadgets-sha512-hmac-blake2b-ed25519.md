...
## Status
Accepted — all gadget milestones (M1–M8) implemented and validated against authoritative
oracles (JCA, cardano-client-lib `Blake2bUtil`, BouncyCastle Ed25519, `BigInteger` mod-p). The
full CIP-1852 derivation circuit reproduces the real Cardano payment key hash in-circuit.
Remaining work is *performance* (at-scale proving via leaner field reduction + windowing + the
blst MSM prover), not correctness — see §6.1 and the M5 cost note.
...
| **M8** | Full CIP-1852 derivation circuit; integrate into `account-ownership-recovery` usecase | ✅ **circuit DONE** — `lib/ed25519/Cip1852Derivation.java` + `Cip1852DerivationTest`; the composed **full `m/1852'/1815'/0'/0/0` path** (3 hardened + 2 soft steps + leaf pubkey + blake2b-224, ~90M constraints) reproduces the exact Cardano payment key hash, validated byte-exact vs cardano-client HD derivation + `Blake2bUtil`. Usecase + on-chain Yaci E2E: ✅ **DONE** in `zeroj-usecases/account-ownership-recovery` — off-chain ownership proof (root key→address pkh, ~90M, in-circuit vs cardano-client) + on-chain recovery gate (provable 829-constraint commitment proof verified on Yaci DevKit by the `RecoveryProofValidator` Julc validator, confirmed with a real tx). Full-derivation *proof* at scale is gated on the M5/M7 optimizations. | 2 wk |
