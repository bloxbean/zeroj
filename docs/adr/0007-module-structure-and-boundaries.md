...
1. **Java developers** who want a standalone ZK verification library (no Yaci, no Cardano)
2. **Yaci network operators** who want proof-verified app-layer consensus
...
### Layer 1: Core ZK Library (standalone, no Cardano/Yaci dependency)
...
### Layer 2: Yaci App-Layer Integration

| Module | Purpose |
|--------|---------|
| `zeroj-yaci-protocol` | `AppProofSubmission` wire format, sequence model, submitter signature |
| `zeroj-yaci-zk` | Submission pipeline, state-root validation, replay protection, policy validator |
| `zeroj-yaci-plugin-api` | `ExecutionPlugin`, `ProvablePluginSpec`, plugin lifecycle |
...
zeroj-yaci-protocol <-- depends on zeroj-api, zeroj-codec
zeroj-yaci-zk       <-- depends on zeroj-yaci-protocol, zeroj-verifier-core
zeroj-yaci-plugin-api <-- depends on zeroj-yaci-protocol

zeroj-cardano       <-- depends on zeroj-api, zeroj-codec
zeroj-ccl           <-- depends on zeroj-cardano, cardano-client-lib
```

**No circular dependencies. No Yaci modules depend on Cardano modules or vice versa.**
...
- `com.bloxbean.cardano.zeroj.yaci.protocol`
...
- **Risk**: Too many modules for a small team. **Mitigation**: Start with core modules only (Milestones 0-2 = 5 modules). Add Yaci/Cardano modules as needed. Empty modules are cheap.
