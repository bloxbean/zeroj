...
| **F** | Hint-based Fe25519 inverse | differential vs Fermat/`BigInteger.modInverse`; soundness mutation suite; adversarial review | ✅ **DONE** — `Fe25519.inverseHint`/`inverseFromCandidate` (prover supplies `a⁻¹`, circuit enforces `a·a⁻¹==1` via the **deterministic** mul, so independent of the CRT-mul audit). **inverse 4,089,908 → 10,757 (~380×).** Wired into `Ed25519Point.encode` under `USE_HINT_INVERSE` — **default ON** (it is a standard, trivially-sound hint; disable with `Fe25519.USE_HINT_INVERSE = false` for a zero-advice Fermat inverse). Adversarial review: SOUND, no forgery. Full derivation: **31.3M → 19.1M** (default, hint inverse) → **13.7M** (+ the still-gated hint mul), pkh-validated vs cardano-client. **The audit-gated CRT mul (`USE_HINT_MUL`) stays default OFF** until external audit. |
| **E** | Hash-gadget optimization (SHA-512 Ch/Maj) | JCA cross-check unchanged; constraint reduction | ✅ **DONE** — `Sha512` Ch = `g+e·(f−g)` (1 mul/bit, was 3), Maj = `ab+c·(a⊕b)` (3, was 5). **SHA-512 129,723 → 109,243 (1.19×)**; **HMAC-SHA512 535,685 → 453,765** (auto). Blake2b unchanged (no Ch/Maj). Still validated byte-exact vs JCA. Closes the SHA gap to ~1.5× circom (deeper Σ/σ work would be needed for parity). |
