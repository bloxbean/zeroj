...
## Status
Proposed — Phase A (windowing) and Phase B (lazy reduction) **done and validated** (combined ~6×
on Ed25519 scalar mult: ~29M → ~4.7M); Phase C (hints + CRT mul) **queued** with an
implementation-ready spec, gated on the soundness tests + external audit.
...
| **C** | General trusted hints + CRT non-native mul | soundness negative tests (mutate hints → reject) + integer-identity argument + audit | **QUEUED** — implementation spec ready: [`0028-phaseC-hints-crt-mul-plan.md`](0028-phaseC-hints-crt-mul-plan.md). Not implemented until the under-constraint tests + external audit clear (pillars 2, 4, 5). |
