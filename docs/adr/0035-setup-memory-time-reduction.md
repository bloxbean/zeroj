...
| **M6a (sparse key store)** | **`-Xmx8g` PASS** (user-measured, macOS host: 6.4 min, sparse setup end-to-end — the heap floor is ≤8 GB). **Chained Docker e2e validation (2026-07-10): setup → prove → verify in ONE container under a hard `--memory=16g --memory-swap=16g` cap (zero swap), `-Xmx8g` for both heavy steps, sparse bundle on a VM-native volume — setup 9.6 min, prove 161 s (proof 150.2 s, r1cs.bin cache hit from setup, self-check PASS), verify VALID 0.17 s, TOTAL 12.6 min, exit 0.** cgroup memory.peak pinned at exactly 16 GiB both phases = page cache filling to the ceiling with reclaim working (anonymous heap ≤8g); completion without OOM-kill is the pass signal. This closes the last gap: prove had never read a *sparse* bundle under a memory cap, and setup had only been capped pre-sparse at 12g. | unchanged (6.3 min) | 2026-07-10: `SparsePointFile` — presence bitmap + per-block rank prefix sums + packed non-infinity points, behind the ADR-0033 reader seams; `format=sparse-v1` in the manifest, absent = dense (old bundles + zkey imports untouched — the live snarkjs round-trip test stays green). 19M point files: **24.2 → 9.30 GB logical** (B1 4.2→0.82, B2 8.4→1.63); prove from the sparse store: 55.8 s, self-check PASS, verify VALID — sparse is the CLI `setup` default (`--dense` opts out). H uses an all-present bitmap (dense by nature; a measure-zero zero scalar reads as infinity via zero limbs). An old CLI reading a sparse bundle fails loudly on the manifest count check. **M6b (point-compressed distribution, ~4.7 GB download) remains open** — G1 x-only compression needs an Fp sqrt (trivial, p ≡ 3 mod 4); G2 needs an Fp2 sqrt (not yet implemented) — an M6b-lite (compress G1 only) would land ~5.5 GB. |
