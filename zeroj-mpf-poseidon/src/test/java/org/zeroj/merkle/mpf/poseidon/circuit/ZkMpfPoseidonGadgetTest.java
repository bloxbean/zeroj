...
import org.zeroj.merkle.mpf.poseidon.ccl.PoseidonMpfCodec;
...
class ZkMpfPoseidonGadgetTest {
    ...
    @Test
    void rejectsNonCanonicalPayloadInPaddingStep() {
        PoseidonMpfTrie trie = PoseidonMpfTrie.inMemory();
        byte[] key = bytes("registry:member:1");
        byte[] value = bytes("active");
        trie.put(key, value);
        ...
        byte[] proof = trie.getProofWire(key).orElseThrow();
        int proofSteps = PoseidonMpfCodec.decode(proof).size();
        int maxSteps = proofSteps + 1;
        PoseidonMpfWitness witness = PoseidonMpfCodec.toWitness(
                key, proof, maxSteps, MAX_FORK_PREFIX_CHUNKS);
        var circuit = inclusionCircuit(maxSteps, MAX_FORK_PREFIX_CHUNKS);
        var inputs = new ZkInputMap()
                .put("root", PoseidonMpfHash.fieldFromDigestBytes(trie.getRootHash()))
                .put("value_commitment", PoseidonMpfValueCommitment.field(value));
        ...
        assertThrows(ArithmeticException.class, () -> circuit.calculateWitness(
                withInput(inputs.toWitnessMap(), "mpf_fork_root_" + proofSteps, BigInteger.ONE),
                CurveId.BLS12_381));
    }
