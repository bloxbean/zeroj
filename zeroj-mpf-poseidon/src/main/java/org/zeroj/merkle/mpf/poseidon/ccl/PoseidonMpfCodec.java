package org.zeroj.merkle.mpf.poseidon.ccl;
...
import org.zeroj.merkle.mpf.poseidon.witness.PoseidonMpfWitness;
...
public final class PoseidonMpfCodec {
    /** MPF paths contain at most one proof step per 256-bit key nibble. */
    public static final int MAX_PROOF_STEPS = PoseidonMpfHash.KEY_PATH_NIBBLES;
    /** Generous bound for 64 canonical branch/leaf/fork steps. */
    public static final int MAX_PROOF_BYTES = 16 * 1024;
    /** Largest accepted primitive CBOR container is a 128-byte sibling blob. */
    private static final int MAX_CBOR_PREALLOCATION = 256;
    ...
    public static PoseidonMpfWitness toWitness(
        ...
        if (maxSteps < 0 || maxSteps > MAX_PROOF_STEPS) {
            throw new IllegalArgumentException(
                    "maxSteps must be in [0, " + MAX_PROOF_STEPS + "]");
        }
        int requiredForkChunks = maxSteps == 0 ? 0 : 2;
        if (maxForkPrefixChunks != requiredForkChunks) {
            throw new IllegalArgumentException("maxForkPrefixChunks must be exactly "
                    + requiredForkChunks + " when maxSteps=" + maxSteps);
        }
    ...
    public static List<Step> decode(byte[] proofCbor) {
        Objects.requireNonNull(proofCbor, "proofCbor");
        if (proofCbor.length == 0 || proofCbor.length > MAX_PROOF_BYTES) {
            throw new IllegalArgumentException(
                    "MPF proof length must be in [1, " + MAX_PROOF_BYTES + "] bytes");
        }
        try {
            CborDecoder decoder = new CborDecoder(new ByteArrayInputStream(proofCbor));
            // The wire-length bound alone is insufficient: CBOR can declare a
            // huge byte string/array in a few bytes and some decoders allocate
            // from that declared length before discovering EOF.
            decoder.setMaxPreallocationSize(MAX_CBOR_PREALLOCATION);
            List<DataItem> items = decoder.decode();
            if (items.size() != 1 || !(items.getFirst() instanceof Array root)) {
                throw new IllegalArgumentException("MPF proof must contain exactly one CBOR array");
            }
            requireUntagged(root, "MPF proof root");
            ByteArrayOutputStream canonical = new ByteArrayOutputStream(proofCbor.length);
            new CborEncoder(canonical).encode(items.getFirst());
            if (!Arrays.equals(proofCbor, canonical.toByteArray())) {
                throw new IllegalArgumentException("MPF proof must use canonical CBOR encoding");
            }
            if (root.getDataItems().size() > MAX_PROOF_STEPS) {
                throw new IllegalArgumentException(
                        "MPF proof exceeds the global " + MAX_PROOF_STEPS + "-step bound");
            }
        ...
        } catch (StackOverflowError | OutOfMemoryError hostileInput) {
            throw new IllegalArgumentException("MPF proof exceeds decoder resource limits", hostileInput);
        } catch (Exception e) {
    ...
    private static Step decodeStep(Array array) {
        ...
        if (tag == null) throw new IllegalArgumentException("MPF proof step is missing its type tag");
        if (tag.hasTag()) {
            throw new IllegalArgumentException("MPF proof step must have exactly one type tag");
        }
        long tagValue = tag.getValue();
        if (tagValue == TAG_BRANCH) {
            if (array.getDataItems().size() != 2) {
                throw new IllegalArgumentException("branch proof step must contain [skip, neighbors]");
            }
        ...
        if (tagValue == TAG_FORK) {
            if (array.getDataItems().size() != 2
                    || !(array.getDataItems().get(1) instanceof Array neighbor)
                    || neighbor.getDataItems().size() != 3) {
                throw new IllegalArgumentException(
                        "fork proof step must contain [skip, [nibble, prefix, root]]");
            }
            requireExactTag(neighbor, TAG_BRANCH, "fork neighbor tuple");
            int skip = readUInt(array.getDataItems().get(0));
        ...
        if (tagValue == TAG_LEAF) {
            if (array.getDataItems().size() != 3) {
                throw new IllegalArgumentException("leaf proof step must contain [skip, keyHash, valueHash]");
            }
            int skip = readUInt(array.getDataItems().get(0));
    ...
    private static int readUInt(DataItem item) {
        requireUntagged(item, "unsigned integer");
    ...
    private static byte[] readBytes(DataItem item) {
        requireUntagged(item, "byte string");
    ...
    private static void requireUntagged(DataItem item, String description) {
        if (item.hasTag()) {
            throw new IllegalArgumentException(description + " must not carry a CBOR semantic tag");
        }
    }

    private static void requireExactTag(DataItem item, long expected, String description) {
        Tag tag = item.getTag();
        if (tag == null || tag.getValue() != expected || tag.hasTag()) {
            throw new IllegalArgumentException(
                    description + " must carry exactly CBOR tag " + expected);
        }
    }
